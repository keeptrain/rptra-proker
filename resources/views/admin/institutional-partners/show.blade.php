<x-datatables :routeName="'prog-mitra.destroy'" :datatablesId="'datatables-partners'" :nameInputId="'partner_ids[]'">
    ...
    <x-slot name="tbody">
        ...
        <tr>
            <td>
                <input type="checkbox" name="partner_ids[]" class="row-checkbox" value="{{ $item->id }}">
            </td>
            <td>{{ $item->name}}</td>
            <td>{{ $item->id}}</td>
            <td>{{ $item->created_at}}</td>
            <td >
                <button onclick="window.location.href='{{ route('prog-mitra.edit', $item->id) }}'" class="p-2 inline-flex items-center rounded-md bg-zinc-200 dark:bg-zinc-900 text-zinc-700 dark:text-zinc-400" type="button">
