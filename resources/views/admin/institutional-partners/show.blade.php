<form action="{{ route('prog-mitra.destroy') }}" method="POST">
    ...
    <x-admin.table :programs="$institutionalPartners">
        ...
        <!-- Slot untuk tbody -->
        <x-slot name="tbody">
            @foreach ($institutionalPartners as $program)
                <tr class="bg-white border-b dark:bg-white dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-50">
                    <td class="w-4 p-4">
                        <div class="flex items-center">
                            <input id="checkbox-all"type="checkbox" name="partner_ids[]" value="{{ $program->id }}"
                    ...
                    <td class="px-6 py-4">
                        <a href="{{ route('prog-mitra.edit', $program->id) }}" class="text-blue-600 dark:text-blue-500 hover:underline">Edit</a>
                    </td>
                </tr>
            @endforeach
        </x-slot>
