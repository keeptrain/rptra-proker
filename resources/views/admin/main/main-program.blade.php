...
@section('content-table')
    @if (Route::currentRouteName() === 'progpokok.show')
        <form action="{{ route('progpokok.destroy') }}" method="POST">
            @csrf
            @method('DELETE')
            <x-admin.table :programs="$mainPrograms">
                <x-slot name="slotbutton">
                    <button type="button" onclick="window.location.href='{{ route('progpokok.create') }}'"
                ...
                <x-slot name="tbody">
                    @foreach ($mainPrograms as $program)
                        <tr
                            ...
                            <td class="w-4 p-4">
                                <div class="flex items-center">
                                    <input id="checkbox-all"type="checkbox" name="main_ids[]"
                                        value="{{ $program->id }}"
    ...
    @elseif (Route::currentRouteName() === 'progpokok.create')
        <x-admin.add-form :route="'progpokok.store'">
            <x-slot name="formBody">
                <!-- Konten form di sini -->
                @include('admin.main.create-program')
            </x-slot>
        </x-admin.add-form>
    @endif
@endsection
