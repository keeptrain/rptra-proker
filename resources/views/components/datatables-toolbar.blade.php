<div class="bg-white dark:bg-transparent inline-flex ">
    ...
    @if (Route::currentRouteName() === 'prog-transaksi.index')
        ...
        <div x-data="{ openExport: false }" class="relative">
            <button @click="openExport = ! openExport" class="pe-10 p-2 border border-gray-200 shadow-sm"
                type="button">
                <span class="select-none absolute inset-y-0 right-0 flex items-center cursor-pointer pr-3 ">
                    <svg :class="{ 'rotate-180': openExport }" class="w-4 h-4 transform transition-transform duration-200"
